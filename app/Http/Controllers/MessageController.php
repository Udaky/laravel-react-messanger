    ...
    public function loadOlder(Message $message){
        if($message->group_id){
            ...
            ->where(function($query) use ($message){
                $query->where('created_at', $message->sender_id)->where('receiver_id', $message->receiver_id)->orWhere('sender_id', $message->receiver_id)->where('receiver_id', $message->sender_id);
            })
            ->latest()
            ->paginate(10);
        }
